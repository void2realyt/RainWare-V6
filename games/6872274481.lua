...
run(function()
    SigridExploit = vape.Categories.Blatant:CreateModule({
        Name = "ElkExploit",
        Tooltip = "Elk now has infinite stamina",
        Function = function(callback)
            if callback then
                repeat 
                    for _ = 1,5 do
                        game:GetService("ReplicatedStorage").rbxts_include.node_modules["@rbxts"].net.out._NetManaged.ElkKitMounted:FireServer()
                        game:GetService("ReplicatedStorage").rbxts_include.node_modules["@rbxts"].net.out._NetManaged.ElkKitDismounted:FireServer()
                    end
                    task.wait(0.5)
                ...
                spawn(function()
                    while Antihit.Enabled do
                        ...
                        if root then
                            ...
                            if foundEnemy then
                                -- Move up
                                root.CFrame = CFrame.new(orgPos + Vector3.new(0, 70, 0))
